      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {activities.map(activity => (
          <div key={activity.id} className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg hover:border-brand-200 transition-all group cursor-pointer"
               onClick={() => dispatch({ type: 'OPEN_MODAL', payload: { type: 'ACTIVITY_DETAIL', data: activity } })}>
            <div className="flex justify-between items-start mb-4">
              <span className={cn(
                "px-3 py-1 rounded-full text-xs font-semibold",
                activity.status === '已结束' ? "bg-slate-100 text-slate-600" :
                activity.status === '进行中' ? "bg-green-50 text-green-600" : "bg-blue-50 text-blue-600"
              )}>
                {activity.status}
              </span>
            ...
            <div className="mt-6 pt-6 border-t border-slate-100 flex justify-between items-center">
              <div className="flex -space-x-2">
                ...
                <div className="w-8 h-8 rounded-full bg-slate-50 border-2 border-white flex items-center justify-center text-[10px] text-slate-400">+5</div>
              </div>
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  dispatch({ type: 'OPEN_MODAL', payload: { type: 'ACTIVITY_DETAIL', data: activity } });
                }}
                ...
                return (
                  <tr key={task.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-slate-400">{task.type} · {activity?.name || '未关联活动'}</div>
                    </td>
                    <td className="px-6 py-4">
                ...
                <tr key={vehicle.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={cn(
                      ...
                      vehicle.status === '执行中' ? "bg-yellow-100 text-yellow-800" : 
                ...
                <tr key={driver.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={cn(
                      ...
                      driver.status === '执行中' ? "bg-yellow-100 text-yellow-800" : 
                      "bg-red-100 text-red-800"
                    )}>
                      {driver.status}
                    </span>
